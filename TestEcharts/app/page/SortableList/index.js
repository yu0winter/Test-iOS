/**
 * Created by luo yu shi @ zhongshu on 2017/7/14.
 * https://github.com/gitim/react-native-sortable-list/blob/master/package.json "version": "0.0.16",
 * @flow
 */
...
export default class SortableList extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        let {sortingEnabled, data: nextData, order: nextOrder} = nextProps;
    ...
    _disableOfIndex(index){
        const {disabledIndex} = this.props;
        const {sortingEnabled} = this.state;

        var disabled = true;
        if (sortingEnabled){
            if (disabledIndex.indexOf(index) >= 0){
                disabled = true;
            } else {
                disabled = false;
            }
    ...
    _renderRows() {
        const {horizontal, rowActivationTime, renderRow} = this.props;
        ...
        return order.map((key, index) => {
            ...
            const disabled = this._disableOfIndex(index);
            return (
                <Row
    ...
    _onUpdateLayouts() {
        ...
        var _rowsLayoutsValues = [];
        for(let key in this._rowsLayouts) {
            _rowsLayoutsValues.push(this._rowsLayouts[key]);
        }

        Promise.all([this._footerLayout, ..._rowsLayoutsValues])
    ...
    _setOrderOnMove() {
        ...
        if (rowUnderActiveKey !== activeRowKey && rowUnderActiveKey !== this._prevSwapedRowKey) {
            ...
            if (isNeighbours) {
                ...
                this.nextOrder = swapArrayElements(order, activeRowIndex, rowUnderActiveIndex);

                this.sortFrom = activeRowIndex;
                this.sortTo = rowUnderActiveIndex;
            } else {
                this.nextOrder = order.slice();
                this.nextOrder.splice(activeRowIndex, 1);
                this.nextOrder.splice(rowUnderActiveIndex, 0, activeRowKey);
             ...
             * 添加不能排序的元素,拖拽的时候也不响应
             */
            if (this._disableOfIndex(rowUnderActiveIndex)){
                return;
            }
    ...
    _onReleaseRow = (rowKey) => {
        ...
        if (this.props.onChangeOrder) {
            const {activeRowKey, activeRowIndex, order} = this.state;
            this.props.onChangeOrder(this.nextOrder, parseInt(activeRowKey, 10), this.sortTo);
        }

        this.nextOrder = undefined;
        this.sortTo = undefined;
    };
